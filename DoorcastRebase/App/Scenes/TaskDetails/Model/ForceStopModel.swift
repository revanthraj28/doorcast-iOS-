...
struct ForceModel : Codable {
    let status : Bool?
    let message : String?
    let data : [finishstopdata]?
    ...
    init(from decoder: Decoder) throws {
        ...
        data = try values.decodeIfPresent([finishstopdata].self, forKey: .data)
        task_status = try values.decodeIfPresent(String.self, forKey: .task_status)
    }

}
struct finishstopdata : Codable {
    let crew_id : String?
